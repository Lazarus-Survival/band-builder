...
function newWeapon(){return {id:uid(),name:"",ammoBlocks:1};}
...
function memberCost(member){const b=band();const p=b.profiles[member.profile];if(!p)return 0;const weaponCost=member.weapons.reduce((sum,weapon)=>sum+(b.weapons[weapon.name]||0)+ammoCost(weapon)+toolCost(weapon),0);const gearCost=member.gear.reduce((sum,item)=>sum+(b.gear[item]||0),0);return p.cost+repeatedTraitCost(member)-repeatedFlawValue(member)+(b.armor[member.armor]||0)+weaponCost+gearCost;}
...
function renderWeaponRows(card,member){ensureLoadoutIds(member);const container=card.querySelector(".weapon-list");container.innerHTML=member.weapons.map((weapon,index)=>{const rule=ammoRule(weapon.name);return `<div class="loadout-row weapon-row"><label>Arma ${index+1}<select class="weapon-select" data-index="${index}">${weaponOptions(weapon.name)}</select></label>${rule?`<label>Munición<select class="ammo-select" data-index="${index}">${ammoOptions(weapon)}</select></label>`:""}${toolUpgradePrice(weapon)!==null?`<label class="tool-option"><input type="checkbox" class="tool-upgrade" data-index="${index}" ${weapon.tool?"checked":""}>Herramienta (+${toolUpgradePrice(weapon)} pts)</label>`:""}${member.weapons.length>1?`<button type="button" class="remove-loadout remove-weapon" data-index="${index}" title="Eliminar arma">×</button>`:""}</div>`;}).join("");}
function renderGearRows(card,member){ensureLoadoutIds(member);const container=card.querySelector(".gear-list");container.innerHTML=member.gear.map((item,index)=>`<div class="loadout-row gear-row"><label>Equipo ${index+1}<select class="gear-select" data-index="${index}">${options(band().gear,item,"Sin equipo")}</select></label>${attachmentSelector(member,item,index)}${member.gear.length>1?`<button type="button" class="remove-loadout remove-gear" data-index="${index}" title="Eliminar equipo">×</button>`:""}</div>`).join("")+toolWeapons(member).map(w=>`<div class="locked-tool"><img src="assets/weapons/arma-cc-pesada.png" width="50" height="50" alt=""><div><b>Herramienta</b><p>${TOOL_TEXT}</p><small>Vinculada a Arma ${member.weapons.indexOf(w)+1} · +${toolCost(w)} pts</small></div></div>`).join("");}
...
function ruleDetails(member){const rows=[];if(member.trait)rows.push(`<div class="rule-line"><b>${escapeHtml(member.trait)}:</b> ${escapeHtml(LAZARUS_DATA.traits[member.trait]?.text||"Consulta el reglamento.")} <em>(${formatPoints(repeatedTraitCost(member))} pts)</em></div>`);if(member.flaw)rows.push(`<div class="rule-line"><b>${escapeHtml(member.flaw)}:</b> ${escapeHtml(LAZARUS_DATA.flaws[member.flaw]?.text||"Consulta el reglamento.")} <em>(-${formatPoints(repeatedFlawValue(member))} pts)</em></div>`);if(member.armor)rows.push(`<div class="rule-line"><b>${escapeHtml(member.armor)}:</b> ${escapeHtml(LAZARUS_DATA.armorText[member.armor]||"")}</div>`);for(const weapon of member.weapons.filter(w=>w.name)){const ammo=ammoDescription(weapon);rows.push(`<div class="rule-line"><b>${escapeHtml(weapon.name)}:</b> ${escapeHtml(weaponRuleText(weapon))}${ammo?` <span class="ammo-note">· ${escapeHtml(ammo)}</span>`:""}</div>`);}for(const item of member.gear.filter(Boolean)){rows.push(`<div class="rule-line"><b>${escapeHtml(item)}:</b> ${escapeHtml(LAZARUS_DATA.gearText[item]||"")}</div>`);}return rows.join("");}
function bindMember(card,member){bindLoadoutExtras(card,member);card.querySelector(".member-name").addEventListener("input",e=>{member.name=e.target.value;renderSummary();});const fields={".member-profile":"profile",".member-trait":"trait",".member-flaw":"flaw",".member-armor":"armor"};for(const[selector,key]of Object.entries(fields)){card.querySelector(selector).addEventListener("change",e=>{member[key]=e.target.value;render();});}card.querySelector(".add-weapon").addEventListener("click",()=>{member.weapons.push(newWeapon());render();});card.querySelectorAll(".weapon-select").forEach(select=>{select.addEventListener("change",e=>{const weapon=member.weapons[Number(e.target.dataset.index)];clearWeaponLinks(member,weapon);weapon.name=e.target.value;weapon.ammoBlocks=1;render();});});card.querySelectorAll(".ammo-select").forEach(select=>{select.addEventListener("change",e=>{member.weapons[Number(e.target.dataset.index)].ammoBlocks=Math.max(1,Number(e.target.value)||1);render();});});card.querySelectorAll(".remove-weapon").forEach(button=>{button.addEventListener("click",e=>{clearWeaponLinks(member,member.weapons[Number(e.currentTarget.dataset.index)]);member.weapons.splice(Number(e.currentTarget.dataset.index),1);if(member.weapons.length===0)member.weapons.push(newWeapon());render();});});card.querySelector(".add-gear").addEventListener("click",()=>{member.gear.push("");render();});card.querySelectorAll(".gear-select").forEach(select=>{select.addEventListener("change",e=>{member.gearTargets[Number(e.target.dataset.index)]="";member.gear[Number(e.target.dataset.index)]=e.target.value;render();});});card.querySelectorAll(".remove-gear").forEach(button=>{button.addEventListener("click",e=>{member.gearTargets.splice(Number(e.currentTarget.dataset.index),1);member.gear.splice(Number(e.currentTarget.dataset.index),1);if(member.gear.length===0)member.gear.push("");render();});});card.querySelector(".remove-member").addEventListener("click",()=>{state.members=state.members.filter(m=>m.id!==member.id);render();});}
...
function sheetMember(member,index){const stats=effectiveStats(member);const details=[];if(member.trait)details.push(`<p><b>Dote — ${escapeHtml(member.trait)}:</b> ${escapeHtml(LAZARUS_DATA.traits[member.trait]?.text||"Consulta el reglamento.")}</p>`);if(member.flaw)details.push(`<p><b>Defecto — ${escapeHtml(member.flaw)}:</b> ${escapeHtml(LAZARUS_DATA.flaws[member.flaw]?.text||"Consulta el reglamento.")}</p>`);if(member.armor)details.push(`<p><b>Protección — ${escapeHtml(member.armor)}:</b> ${escapeHtml(LAZARUS_DATA.armorText[member.armor]||"")}</p>`);for(const weapon of member.weapons.filter(w=>w.name)){const ammo=ammoDescription(weapon);details.push(`<p><b>Arma — ${escapeHtml(weapon.name)}:</b> ${escapeHtml(weaponRuleText(weapon))}${ammo?`<br><span class="sheet-ammo"><b>${escapeHtml(ammo)}</b></span>`:""}</p>`);}for(const item of member.gear.filter(Boolean)){details.push(`<p><b>Equipo — ${escapeHtml(item)}:</b> ${escapeHtml(LAZARUS_DATA.gearText[item]||"")}</p>`);}return `<section class="sheet-member"><div class="sheet-member-head"><h4>${escapeHtml(member.name||`${member.profile} ${index+1}`)} · ${escapeHtml(member.profile)}</h4><b>${formatPoints(memberCost(member))} pts</b></div><div class="sheet-stats">${STAT_NAMES.map((name,i)=>`<div><b>${stats[i]??"-"}</b>${name}</div>`).join("")}</div><div class="sheet-details">${details.join("")}</div></section>`;}
