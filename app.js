...
function renderWeaponRows(card,member){const container=card.querySelector(".weapon-list");container.innerHTML=member.weapons.map((weapon,index)=>{const rule=ammoRule(weapon.name);return `<div class="loadout-row weapon-row"><label>Arma ${index+1}<select class="weapon-select" data-index="${index}">${options(band().weapons,weapon.name,"Sin arma")}</select></label>${rule?`<label>Munición<select class="ammo-select" data-index="${index}">${ammoOptions(weapon)}</select><span class="loadout-help">Base: ${rule.size} incluidos · cada +${rule.size}: ${rule.cost} pts</span></label>`:""}${member.weapons.length>1?`<button type="button" class="remove-loadout remove-weapon" data-index="${index}" title="Eliminar arma">×</button>`:""}</div>`;}).join("");}
